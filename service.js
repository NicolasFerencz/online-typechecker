...
const removeANSIColors = (str) => str.replace(
    ...
    try {
      socket.on(ev.name, (arg) => {
        ...
        command.stdout.on('data', (data) => {
          if (!fs.existsSync(`${localDir}/${uuid}${ev.fileSubfix}`)) {
            socket.emit('input', {
              code: removeANSIColors(data.toString()),
              isElixir: false,
            });
        ...
        command.stdout.on('close', () => {
          fs.unlink(filePath, () => {});
          if (ev.fileSubfix) {
            const auxFile = `${localDir}/${uuid}${ev.fileSubfix}`;
